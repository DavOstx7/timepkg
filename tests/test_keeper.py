...
def test_result_casting(value_factory, positive_float_factory):
    ...
    assert (return_value, execution_time) == result.tuple()
    assert {"return_value": return_value, "execution_time": execution_time} == result.dict()


def test_parameterless_function(value_factory, testable_duration_factory):
    return_value, sleep_duration = value_factory(), testable_duration_factory()
    ...
    assert result.return_value == return_value
    assert valid_execution_time(sleep_duration, result.execution_time, VALID_ERROR_MARGIN_TIME)


def test_parameterized_function(testable_duration_factory):
    sleep_duration = testable_duration_factory()
    _args = (1, 2, 3)
    _kwargs = {"d": 4, "e": 5, "f": 6}
